...
import { Search, SlidersHorizontal, RotateCcw, ArrowLeft } from 'lucide-react';
...
import { provinceOf } from '../../data/geo';
...
const CitySpots = () => {
  ...
  const clearFilters = () => {
    setSelActivities([]); setElevBand('any'); setGemsOnly(false); setSort('featured'); setQuery('');
  };
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        setError('Failed to load this destination. Please try again later.');
      } finally {
        setIsLoading(false);
      }
    })();
  ...
  const cityData = useMemo(() => allSpots.find((s) => s.city === cityName) || null, [allSpots, cityName]);
  ...
  const heroImg = cityPhotos[cityName] || cityData?.heroImage || places[0]?.picture || 'https://picsum.photos/seed/mm-region/1600/900';
  const tagline = cityData?.tagline || cityTaglines[cityName] || '';
  ...
  return (
    <div className="min-h-screen bg-night-950 text-white selection:bg-lime-400 selection:text-night-950">
      ...
      <main className="mx-auto max-w-7xl space-y-8 px-4 pb-8 pt-6 sm:px-6 lg:space-y-10">
        ...
        >
          <img loading="lazy" decoding="async" src={heroImg} alt={cityName} className="absolute inset-0 h-full w-full object-cover" />
          <div className="absolute inset-0 bg-gradient-to-t from-night-950 via-night-950/50 to-night-950/20" />
          <div className="relative flex min-h-[320px] flex-col justify-end px-6 py-10 sm:min-h-[400px] sm:px-12 sm:py-14">
            <Eyebrow>{provinceOf(cityName)}</Eyebrow>
            <h1 className="mt-3 text-4xl font-extrabold tracking-tight text-white sm:text-6xl">{cityName}</h1>
            {tagline && <p className="mt-3 max-w-xl text-white/70">{tagline}</p>}
          </div>
        </motion.section>
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
            ...
            <button
              ...
              className={`inline-flex shrink-0 items-center justify-center gap-2 rounded-2xl border px-5 py-3 text-sm font-semibold transition-colors ${
                showFilters || activeCount ? 'border-lime-400/50 bg-lime-400/10 text-lime-300' : 'border-white/10 bg-night-800 text-white/70 hover:border-lime-400/40'
              }`}
            >
              <SlidersHorizontal className="h-4 w-4" /> Filters
              {activeCount > 0 && (
                <span className="flex h-5 min-w-[20px] items-center justify-center rounded-full bg-lime-400 px-1 text-[11px] font-bold text-night-950">{activeCount}</span>
              )}
            </button>
          </div>
                ...
                <Tile pad="p-5 sm:p-6">
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {allActivities.length ? allActivities.map((a) => (
                          <Chip key={a} active={selActivities.includes(a)} onClick={() => toggleActivity(a)}>{a}</Chip>
                        )) : <span className="text-sm text-white/40">No activities listed here.</span>}
                      </div>
                    </div>
                    <div>
                      <p className="mb-2.5 text-xs font-bold uppercase tracking-wider text-white/50">Elevation</p>
                      <div className="flex flex-wrap gap-2">
                        {ELEV_OPTS.map(([v, l]) => <Chip key={v} active={elevBand === v} onClick={() => setElevBand(v)}>{l}</Chip>)}
                      </div>
                    </div>
                  </div>
                  <div className="mt-6 flex flex-col gap-4 border-t border-white/8 pt-5 sm:flex-row sm:items-center">
                    <div>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {SORT_OPTS.map(([v, l]) => <Chip key={v} active={sort === v} onClick={() => setSort(v)}>{l}</Chip>)}
                      </div>
                    </div>
                    <div className="flex items-end gap-3 sm:ml-auto">
                      <Chip active={gemsOnly} onClick={() => setGemsOnly((g) => !g)}>Hidden gems only</Chip>
                      <button onClick={clearFilters} className="inline-flex items-center gap-1.5 rounded-full px-3 py-2 text-sm font-semibold text-white/50 transition-colors hover:text-lime-400">
        ...
        <section>
          ...
          {error ? (
            <Tile className="py-16 text-center"><p className="text-white/70">{error}</p></Tile>
          ) : isLoading ? (
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
              {Array.from({ length: 6 }).map((_, i) => (
                <div key={i} className={`animate-pulse rounded-[1.4rem] border border-white/[0.07] bg-night-800 ${i === 0 ? 'sm:col-span-2 sm:row-span-2 min-h-[280px] sm:min-h-[300px]' : 'min-h-[200px]'}`} />
              ))}
            </div>
          ) : !cityData ? (
            <Tile className="flex flex-col items-center py-16 text-center">
              <p className="text-white/60">We don't have "{cityName}" yet.</p>
              <Link to="/destinations" className="mt-4 rounded-full bg-lime-400 px-5 py-2.5 text-sm font-bold text-night-950 transition-transform hover:-translate-y-0.5">Back to destinations</Link>
            </Tile>
          ) : filtered.length === 0 ? (
            <Tile className="flex flex-col items-center py-16 text-center">
              <p className="text-white/60">No spots match your search &amp; filters.</p>
              <button onClick={clearFilters} className="mt-4 inline-flex items-center gap-1.5 rounded-full bg-lime-400 px-5 py-2.5 text-sm font-bold text-night-950 transition-transform hover:-translate-y-0.5">
            ...
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
              {filtered.map((place, i) => (
                <CityCard
                  key={place._id}
                  name={place.name} location={place.location} picture={place.picture}
                  city={cityName} spotId={place._id} hiddenGem={place.hiddenGem} curatedBy={place.curatedBy}
                  className={spanFor(i, filtered.length)} delay={Math.min(i, 6) * 0.05}
                />
              ))}
            </div>
          )}
