 ...
 * an optional "curated by" credit, and a heart to save/un-save the spot.
 *
...
const CityCard = ({
  ...
  const [saved, setSaved] = useState(() => isSaved(spotId));

  useEffect(() => subscribeSaved(() => setSaved(isSaved(spotId))), [spotId]);

  const onToggleSave = (e) => {
    e.preventDefault();
    e.stopPropagation();
    setSaved(toggleSaved(spotId));
  };
