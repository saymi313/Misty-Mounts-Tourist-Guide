...
/** Travel-agency profile — business details shown to travellers on tour pages. */
export default function AgencyProfile() {
  const { user, updateUser } = useAuth();
  const cities = useCities();
  const [form, setForm] = useState({ agencyName: "", name: "", email: "", phone: "", city: "", bio: "" });
    ...
    (async () => {
      try {
        if (LIVE) {
          ...
          if (!alive) return;
          setForm({
            agencyName: u.agencyName || "", name: u.name || "", email: u.email || "",
  ...
  const initial = (form.agencyName || form.name || "A").charAt(0).toUpperCase();

  if (loading) {
    return (
      <TravelAgencyLayout greeting="My profile" subtitle="Manage your agency details">
  ...
  return (
    <TravelAgencyLayout greeting="My profile" subtitle="Manage your agency details and how travellers see you">
      <div className="grid gap-6 lg:grid-cols-[minmax(0,0.9fr)_1.4fr]">
        <Card className="h-fit">
          <div className="flex items-center gap-4">
            {avatar ? (
              <img loading="lazy" decoding="async" src={avatar} alt={form.agencyName} className="h-16 w-16 shrink-0 rounded-2xl object-cover" />
            ) : (
            ...
            <div className="min-w-0">
              <h2 className="truncate text-lg font-bold text-slate-900">{form.agencyName || form.name || "Your agency"}</h2>
              ...
              <span className="mt-1.5 inline-flex items-center gap-1 rounded-full bg-violet-50 px-2.5 py-1 text-xs font-semibold text-violet-600">
                <Compass className="h-3 w-3" /> Travel Agency
              </span>
            </div>
          </div>
          <div className="mt-4">
        ...
        <Card>
          <SectionHead title="Edit your info" sub="Travellers see this on your tour pages." />
          <form onSubmit={handleSave} noValidate className="space-y-5">
            <Field label="Agency name" required value={form.agencyName} onChange={(v) => update("agencyName", v)} placeholder="e.g. Northern Trails Travel Co." error={errors.agencyName} />
            <div className="grid gap-5 sm:grid-cols-2">
              ...
              <Field label="City" hint={cityOptions.length ? "Where your agency is based." : "Ask an admin to add cities."}>
            ...
            <div>
              <label className="mb-1.5 flex items-center gap-1 text-xs font-semibold uppercase tracking-wide text-slate-500">About your agency</label>
              <textarea
                rows="4"
                value={form.bio}
                onChange={(e) => update("bio", e.target.value)}
                maxLength={500}
                placeholder="Tell travellers about your tours, experience and what makes your trips special…"
