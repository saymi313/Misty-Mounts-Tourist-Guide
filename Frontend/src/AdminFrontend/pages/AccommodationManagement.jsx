...
const AccommodationManagement = () => {
  ...
  const fetchAccommodations = async () => {
    try {
      const response = await axios.get("http://localhost:5000/api/admin/accommodations");
  ...
  const handleAddAccommodation = async (newAccommodation) => {
    try {
      await axios.post("http://localhost:5000/api/admin/accommodations", newAccommodation);
  ...
  const handleUpdateAccommodation = async (updatedAccommodation) => {
    ...
    try {
      await axios.put(`http://localhost:5000/api/admin/accommodations/${selectedAccommodation._id}`, updatedAccommodation);
          ...
          <ul>
            {accommodations.map((accommodation) => (
              <li key={accommodation._id}>
                ...
                <p>Availability: {accommodation.isAvailable ? "Available" : "Not Available"}</p>
                <p>Special Offer: {accommodation.specialOffer ? accommodation.specialOffer : "No Offer"}</p>
                <button onClick={() => handleSelectAccommodationForUpdate(accommodation)}>Update</button>
                <button onClick={() => handleDeleteAccommodation(accommodation._id)}>Delete</button>
              </li>
            ))}
          </ul>
